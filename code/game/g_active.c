...
void SpectatorThink(gentity_t *ent, usercmd_t *ucmd)
{
    ...
    if (client->sess.spectatorState != SPECTATOR_FOLLOW)
    {
        ...
        // OSP
        OSP_SpectatorThink(ent, osp.awardsStarted);
    ...
    if (client->sess.spectatorState != SPECTATOR_FOLLOW && g_forceFollow.integer && !OSP_IsClientAdmin(ent - g_entities, qtrue))
    // END
    {
    ...
    else if ((!g_forceFollow.integer || OSP_IsClientAdmin(ent - g_entities, qtrue)) && ucmd->upmove > 0 && (client->ps.pm_flags & PMF_FOLLOW))
    // END
    {
        G_StopFollowing(ent);
    }
    else if ((client->buttons & BUTTON_USE) && !(client->oldbuttons & BUTTON_USE))
    {
        ...
        if (client->sess.spectatorState != SPECTATOR_FOLLOW)
        {
            ...
            client->sess.spectatorFirstPerson = (g_forceFollow.integer < 2 || OSP_IsClientAdmin(ent - g_entities, qtrue)) ? qfalse : qtrue;
            // END
            Cmd_FollowCycle_f(ent, -1);
        }
        ...
        else if (client->sess.spectatorFirstPerson)
        {
            ...
            if (g_forceFollow.integer < 2 || OSP_IsClientAdmin(ent - g_entities, qtrue))
            // END
            {
...
void ClientThink_real(gentity_t *ent)
{
    ...
    else
    {
        ...
        {
            trap_SendServerCommand(ent - g_entities, "cp \"Your jail time has now expired!\n\"");
            client->ospClient->jailTime = 0;
        ...
        if (G_IsClientSpectating(client))
        {
            if (!OSP_SpectatorInactivity(ent))
            {
                return;
            }
    ...
    // OSP
    pm.x174 = client->ospClient->x94;
    pm.levelTime = level.time;
    pm.useKeyForLadders = client->ospClient->useKeyForLadders;
    pm.ospModVersion = osp_modversion.integer;
    // END

    Pmove(&pm);

    // OSP
    client->ospClient->x94 = pm.x174;
    // END
    ...
    // OSP
    if (g_radarFix.integer)
    {
        if (ent->client->sess.team == TEAM_RED || ent->client->sess.team == TEAM_BLUE)
        {
            sub_0002b4b4(ent->client->sess.team);
            ent->r.broadcastClients[0] = osp.x646e3c[ent->client->sess.team][0];
            ent->r.broadcastClients[1] = osp.x646e3c[ent->client->sess.team][1];
        }
    }
    else
    {
        ent->r.broadcastClients[0] = 0;
        ent->r.broadcastClients[1] = 0;
    }
    // END
...
void ClientThink(int clientNum)
{
    ...
    if (!(ent->r.svFlags & SVF_BOT) && !g_synchronousClients.integer)
    {
        ...
        if (ent->client->ospClient->motdTime)
        {
            if (ent->client->ospClient->motdTime < level.time)
            {
                trap_SendServerCommand(ent - g_entities,
                                       va("cp \"@%s\n%s\n%s\n\n%s\n%s\n%s\n%s\n%s\"",
                                          "^GSoldier of Fortune 2 OSP 1.1",
                                          "^H1.1 ^I- ^HJuly 13, 2006",
                                          server_motd0.string,
                                          server_motd1.string,
                                          server_motd2.string,
                                          server_motd3.string,
        ...
        if (ent->client->pers.cmd.forwardmove ||
            ...
            (ent->client->pers.cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_USE)))
        {
