...
{
    if ((self->spawnflags & 1) && activator->client && activator->client->sess.team != TEAM_RED)
    {
        return;
    }
    if ((self->spawnflags & 2) && activator->client && activator->client->sess.team != TEAM_BLUE)
    {
        return;
    }
...
{
    // OSP
    if (osp.altmapFlags & 0x10 && !osp.x6480a0)
    {
        G_FreeEntity(self);
        return;
    }
