...
{
    // OSP
    if ((osp.altmapFlags & 0x40) && !osp.x6480a0)
    {
        G_FreeEntity(ent);
        return;
    }
...
void G_GlassDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod, int hitLocation, vec3_t hitDir)
{
    ...
    // OSP
    vec3_t origin;
    static int osp_glassBreakTime = 0;
    static vec3_t osp_lastGlassBreak = {0, 0, 0};
    // END

    if (sv_nolagGlassBreaks.integer)
    {
        VectorSubtract(self->r.absmax, self->r.absmin, origin);
        VectorScale(origin, 0.5, origin);
        VectorAdd(self->r.absmax, origin, origin);

        if (osp_glassBreakTime > level.time)
        {
            if (Distance(osp_lastGlassBreak, origin) < 500)
            {
                trap_UnlinkEntity(self);
                return;
            }
        ...
        osp_glassBreakTime = level.time + 100;
        VectorCopy(origin, osp_lastGlassBreak);
    }
    // END
