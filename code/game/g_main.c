...
vmCvar_t g_msgs;
vmCvar_t g_msgpos;
vmCvar_t g_randomTeams;
// END

static cvarTable_t gameCvarTable[] = {
	// don't override the cheat state set by the system
	{&g_cheats, "sv_cheats", "", 0, 0.0, 0.0, 0, /* OSP */ /* qfalse */ qtrue /* END */},
	...
	{NULL, "gamedate", "Jul 13 2006", CVAR_ROM, 0.0, 0.0, 0, qfalse},
	// END
	...
	// OSP
	{&g_msgs, "g_msgs", "0", 0, 0.0f, 0.0f, 0, qfalse},
	{&g_msgpos, "g_msgpos", "0", 0, 0.0f, 0.0f, 0, qfalse},
	{&g_randomTeams, "g_randomTeams", "0", CVAR_ARCHIVE | CVAR_SERVERINFO, 0.0f, 0.0f, 0, qfalse},
	// END
	...
	{&g_doWarmup, "g_doWarmup", "0", /* OSP */ /* 0 */ CVAR_ARCHIVE | CVAR_LATCH /* END */, 0.0, 0.0, 0, qtrue},
	...
	{&g_backpackRespawn, "g_backpackrespawn", "40", /* OSP */ /* 0 */ CVAR_SERVERINFO | CVAR_LATCH /* END */, 0.0, 0.0, 0, qtrue},
	{&g_forcerespawn, "g_forcerespawn", /* OSP */ /* "20" */ "5" /* END */, 0, 0.0, 0.0, 0, qtrue},
	...
	{&g_smoothClients, "g_smoothClients", "1", 0, 0.0f, 0.0f, 0, qtrue},
	{&pmove_fixed, "osp_fixedhz", "1", CVAR_ARCHIVE | CVAR_SERVERINFO, 0.0f, 1.0f, 0, qtrue},
	{&pmove_msec, "pmove_msec", "8", CVAR_CHEAT | CVAR_INTERNAL | CVAR_LOCK_RANGE, 8.0f, 8.0f, 0, qtrue},
	// END
	...
	{&g_respawnInterval, "g_respawnInterval", "15", CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_LATCH, 0.0, 0.0, 0, qtrue},
	{&g_respawnInvulnerability, "g_respawnInvulnerability", "5", CVAR_ARCHIVE | CVAR_SERVERINFO, 0.0, 0.0, 0, qtrue},
	// END
	...
	{&g_timeouttospec, "g_timeouttospec", "15", CVAR_ARCHIVE, 0.0, 0.0, 0, /* OSP */ /* qfalse */ qtrue /* END */},
	{&g_roundtimelimit, "g_roundtimelimit", "5", CVAR_ARCHIVE, 0.0, 0.0, 0, /* OSP */ /* qfalse */ qtrue /* END */},
	{&g_roundjointime, "g_roundjointime", "5", CVAR_ARCHIVE, 0.0, 0.0, 0, /* OSP */ /* qfalse */ qtrue /* END */},
	{&g_timeextension, "g_timeextension", "15", CVAR_ARCHIVE, 0.0, 0.0, 0, /* OSP */ /* qfalse */ qtrue /* END */},
	...
	// OSP
	{NULL, "disable_pickup_weapon_US_SOCOM_SILENCED", "0", CVAR_ARCHIVE | CVAR_LATCH, 0.0, 0.0, 0, qfalse},
	// END
	...
	{&g_forceFollow, "g_forceFollow", "0", CVAR_ARCHIVE /* OSP */ | CVAR_LATCH /* END */, 0.0, 0.0, 0, qfalse},
	{&g_followEnemy, "g_followEnemy", "1", CVAR_ARCHIVE /* OSP */ | CVAR_LATCH /* END */, 0.0, 0.0, 0, qfalse},

	{&g_mapcycle, "sv_mapcycle", "none", CVAR_ARCHIVE, 0.0, 0.0, 0, qfalse},

	{&g_pickupsDisabled, "g_pickupsDisabled", "0", CVAR_ARCHIVE | CVAR_LATCH, 0.0, 0.0, 0, /* OSP */ /* qfalse */ qtrue /* END */},

	{&g_suicidePenalty, "g_suicidePenalty", "-1", CVAR_ARCHIVE, 0.0f, 0.0f, 0, /* OSP */ /* qfalse */ qtrue /* END */},
	...
	{&g_voiceTalkingGhosts, "g_voiceTalkingGhosts", "0", CVAR_ARCHIVE, 0.0f, 0.0f, 0, qfalse},
};
...
void G_RegisterCvars(void)
{
	...
	// OSP
	ospRemapped = OSP_RegisterCvars();

	if (!remapped && ospRemapped)
	{
		remapped = qtrue;
	}
	// END
...
*/
// TODO
// osp: some really weird shit going on here
void G_UpdateCvars(void)
{
	...
	if (xe0c >= gameCvarTableSize)
	{
		xe0c = 0;
	}

	if (xe0c + 10 >= gameCvarTableSize)
	{
		max = gameCvarTableSize;
	}
	else
	{
		max = xe0c + 10;
	...
	//for (i = 0, cv = gameCvarTable; i < gameCvarTableSize; i++, cv++)
	for (i = xe0c, cv = &gameCvarTable[xe0c]; i < max; i++, cv++)
	{
		if (cv->vmCvar)
		{
...
void G_UpdateAvailableWeapons(void)
{
	...
	for (weapon = WP_KNIFE; weapon < WP_NUM_WEAPONS; weapon++)
	{
		...
		{
			switch ((int)trap_Cvar_VariableValue(va("disable_sof2pp_%s", item->classname)))
			{
			case 0:
				available[weapon - 1] = '2';
				break;
...
void G_SetGametype(const char *gametype)
{
	...
	// OSP
	//if ((level.gametype == -1) || (!G_DoesMapSupportGametype(gametype) && (!bg_gametypeData[level.gametype].basegametype || !G_DoesMapSupportGametype(bg_gametypeData[level.gametype].basegametype))))
	if ((!G_DoesMapSupportGametype(gametype) && !G_DoesMapSupportGametype(bg_gametypeData[level.gametype].basegametype)) || level.gametype == -1)
	// END
	{
		int i = 0;
...
void G_InitGame(int levelTime, int randomSeed, int restart)
{
	...
	if (osp_modversion.integer >= 2)
	{
		BG_ParseInviewFile(level.pickupsDisabled, qtrue, "data/sof2++.wpn");
	}
	else
	{
		BG_ParseInviewFile(level.pickupsDisabled, qfalse, osp_weaponsFile.string);
	}
...
void CalculateRanks(void)
{
	...
	memset(level.sortedClients, 0, sizeof(level.sortedClients));
	// END
		...
		if (level.clients[i].pers.connected != CON_DISCONNECTED)
		{
			...
			if (level.clients[i].sess.team != TEAM_SPECTATOR)
			{
				...
				if (level.clients[i].pers.connected == CON_CONNECTED)
				{
					...
					// OSP
					if (g_entities[i].client->ps.persistant[PERS_SCORE] > osp.x6479ac)
					{
						osp.x6479ac = g_entities[i].client->ps.persistant[PERS_SCORE];
					}
					// END
				}
			}
	...
	switch (g_alternateSort.integer)
	{
	case 1:
		qsort(level.sortedClients, level.numConnectedClients, sizeof(level.sortedClients[0]), OSP_KDRSort);
		break;
	case 2:
		qsort(level.sortedClients, level.numConnectedClients, sizeof(level.sortedClients[0]), OSP_KPMSort);
		break;
	default:
		qsort(level.sortedClients, level.numConnectedClients, sizeof(level.sortedClients[0]), SortRanks);
	}
...
void SendScoreboardMessageToAllClients(void)
{
	...
	for (i = 0; i < level.maxclients; i++)
	{
		if (level.clients[i].pers.connected == CON_CONNECTED)
		{
			...
			if (level.clients[i].ospClient && level.clients[i].ospClient->ospScoreboard)
			{
				OSP_ScoreboardMessage(g_entities + i);
			}
			else
			{
				DeathmatchScoreboardMessage(g_entities + i);
...
void ExitLevel(void)
{
	...
	gclient_t *cl;
	// OSP
	static qboolean restarted = qfalse;

	if (match_fastRestart.integer || !g_mapcycle.string[0] || !strcmp(g_mapcycle.string, "none"))
	{
		if (!restarted)
		{
...
void CheckIntermissionExit(void)
{
	...
	int readyMask;
	// OSP
	static qboolean statsPrinted = qfalse;
	static qboolean teamsSwapped = qfalse;
	static qboolean teamsShuffled = qfalse;
	int x1c;
	// END
	...
	// OSP
	if (level.time > level.intermissiontime + 2500 && !statsPrinted)
	{
		OSP_PrintStatsToAllClients();
		statsPrinted = qtrue;
	}
	...
	if (g_awards.integer)
	{
		...
		if (g_awards.integer != 2)
		{
			OSP_PrintBadAwards();
		}
	}

	// Don't know wtf is happening here
	if (level.time > level.intermissiontime + g_awards.integer)
	{
		x1c = 19000;
	}
	else
	...
	if (x1c)
	{
		...
		if (match_shuffle.integer && !match_swap.integer)
		{
			if (!teamsShuffled)
			{
				teamsShuffled = qtrue;
	...
	if (level.time < level.intermissiontime + ((g_awards.integer == 1) ? 30000 : ((g_awards.integer == 2) ? 20000 : 10000)))
	// END
	{
		return;
	}
	...
	if (g_scorelimit.integer)
	{
		if (level.gametypeData->teams)
		{
			if (level.teamScores[TEAM_RED] >= g_scorelimit.integer)
			{
				...
				LogExit(va("%s hit the score limit.", OSP_GetTeamname(TEAM_RED)));
				// END
				return;
			}
			...
			if (level.teamScores[TEAM_BLUE] >= g_scorelimit.integer)
			{
				...
				LogExit(va("%s hit the score limit.", OSP_GetTeamname(TEAM_BLUE)));
				// END
				return;
			}
		}
...
void CheckWarmup(void)
{
	...
	{
		// OSP
		if (osp.x6479a0 >= RT_NORMAL)
		{
			level.gametypeData->respawnType = osp.x6479a0;
			level.bodySinkTime = osp.x6479a4;
		}
		// END
...
void G_RunFrame(int levelTime)
{
	...
	// OSP
	// check for matchphase advancement
	OSP_CheckMatchphase();

	// send new infos for team overlays
	OSP_UpdateTeamOverlay();
	// END
