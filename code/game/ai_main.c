...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart)
{
	...
	if (!botstates[client])
		botstates[client] = B_Alloc(sizeof(bot_state_t)); //G_Alloc(sizeof(bot_state_t));
...
int BotHasAssociated(bot_state_t *bs, wpobject_t *wp)
{
	...
	else if (as->item->giType == IT_AMMO)
	{
		...
		if (AMMOWRAP(bs->cur_ps, as->item->giTag) > 10)
		// END
		{
			return 1;
		}
...
int ShouldSecondaryFire(bot_state_t *bs, vec3_t eorg, vec3_t dir)
{
	...
	if (AMMOWRAP(bs->cur_ps, attack->ammoIndex) < 1 || 0 == attack->damage)
	// END
	{
...
int BotTryAnotherWeapon(bot_state_t *bs)
{
	...
	while (i < WP_NUM_WEAPONS)
	{
		...
		if ((AMMOWRAP(bs->cur_ps, weaponData[i].attack[ATTACK_NORMAL].ammoIndex) > 1 || bs->cur_ps.clip[ATTACK_NORMAL][i] > 1) && (bs->cur_ps.stats[STAT_WEAPONS] & (1 << i)))
		// END
		{
			bs->virtualWeapon = i;
...
int BotSelectIdealWeapon(bot_state_t *bs)
{
	...
	while (i < WP_NUM_WEAPONS)
	{
		...
		//	(bs->cur_ps.stats[STAT_WEAPONS] & (1 << i)))
		if ((AMMOWRAP(bs->cur_ps, weaponData[i].attack[ATTACK_NORMAL].ammoIndex) > 1 || bs->cur_ps.clip[ATTACK_NORMAL][i] > 1) &&
			(bs->botWeaponWeights[i] > bestweight) &&
			(bs->cur_ps.stats[STAT_WEAPONS] & (1 << i)))
		// END
		{
...
void StandardBotAI(bot_state_t *bs, float thinktime)
{
	...
	if ((AMMOWRAP(bs->cur_ps, weaponData[bs->cur_ps.weapon].attack[ATTACK_NORMAL].ammoIndex) < 1 && bs->cur_ps.clip[ATTACK_NORMAL][bs->cur_ps.weapon] < 1))
	// END
	{
	...
	if (bs->timeToReact < level.time && bs->currentEnemy && bs->enemySeenTime > level.time + (ENEMY_FORGET_MS - (ENEMY_FORGET_MS * 0.2)))
	{
		...
		if ((CAT_GRENADE == weapon->category ||
			 WP_MM1_GRENADE_LAUNCHER == bs->cur_ps.weapon) &&
			bs->launchAngle < 90)
		{
