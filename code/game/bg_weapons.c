...
char *bg_weaponNames[WP_NUM_WEAPONS] = {
    ...
    "US SOCOM SILENCED", // WP_USSOCOM_SILENCED,
    "OICW",              // WP_OICW,
...
// osp: changed, complete? unverified
static qboolean BG_ParseAttackStats(int weaponNum, attackData_t *attack, void *attacksub, qboolean pickupsDisabled)
{
    ...
    // OSP
    if (weaponNum >= WP_USSOCOM_SILENCED)
    {
        attack->mod = (meansOfDeath_t)weaponNum + 9;
    }
    // END
    ...
    if (sub)
    {
        ...
        for (i = 0; i < 5; i++)
        {
            ...
            // OSP
            else if (0 == Q_stricmp("20mm", tmpStr))
                attack->weaponFlags |= (1 << WP_FIREMODE_20MM);
            // END
            else
                attack->weaponFlags |= (1 << WP_FIREMODE_SINGLE);
        }
...
qboolean BG_InitWeaponStats(qboolean pickupsDisabled, char *weaponsFile)
// END
{
    ...
    GP2 = NULL;
    if (weaponsFile)
        GP2 = trap_GP_ParseFile(weaponsFile, qtrue, qfalse);
    if (!GP2)
        GP2 = trap_GP_ParseFile("ext_data/sof2.wpn", qtrue, qfalse);
...
// END
{
    ...
    {
        trap_GP_Delete(&GP2);
        GP2 = trap_GP_ParseFile("data/sof2++.inview", qtrue, qfalse);

        if (!GP2)
        {
            return BG_InitWeaponStats(pickupsDisabled, weaponsFile);
        }
...
int BG_GetMaxAmmo(const playerState_t *ps, int ammoIndex)
{
    ...
    for (ammo = 0, weapon = WP_KNIFE; weapon < WP_NUM_WEAPONS; weapon++)
    {
        ...
        if (weaponData[weapon].attack[ATTACK_NORMAL].ammoIndex == ammoIndex)
        {
            ...
            // OSP
            if (ps->stats[STAT_OSP_E] == ammoIndex)
            {
                ammo += weaponData[weapon].attack[ATTACK_NORMAL].clipSize * 2.0;
            }
            // END
