...
qboolean G_CallSpawn(gentity_t *ent)
{
    ...
    {
        // OSP
        if ((osp.altmapFlags & 0x100) && !strcmp(ent->classname, "pickup_backpack") && !osp.x6480a0)
        {
            return qfalse;
        }

        if ((osp.altmapFlags & 0x20) && !Q_strncmp(ent->classname, "pickup_", 7) && !osp.x6480a0)
        {
            return qfalse;
        }
        // END
        ...
        {
            // OSP
            if (osp_modversion.integer < 2 && item - bg_itemlist > MODELINDEX_THERMAL)
            {
                return qfalse;
            }
            // END
...
void SP_worldspawn(void)
{
    ...
    if (level.gametypeData->teams)
    {
        ...
        if (text && *text)
        {
            ...
            if (strstr(osp.altmapPath, "mp_hos1"))
            {
                level.gametypeTeam[TEAM_RED] = trap_VM_LocalStringAlloc("hospital");
            }
            else
            {
                level.gametypeTeam[TEAM_RED] = trap_VM_LocalStringAlloc(text);
        ...
        if (text && *text)
        {
            ...
            if (strstr(osp.altmapPath, "mp_hos1"))
            {
                level.gametypeTeam[TEAM_BLUE] = trap_VM_LocalStringAlloc("prometheus");
            }
            else
            {
                level.gametypeTeam[TEAM_BLUE] = trap_VM_LocalStringAlloc(text);
