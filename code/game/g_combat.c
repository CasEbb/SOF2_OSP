...
void TossClientItems(gentity_t *self)
{
    ...
    if (weapon > WP_KNIFE && weapon < WP_NUM_WEAPONS &&
        (AMMOWRAP(self->client->ps, weaponData[weapon].attack[ATTACK_NORMAL].ammoIndex) + self->client->ps.clip[weapon]))
    // END
    {
    ...
    vec3_t hitDir)
{
    ...
    {
        OSP_PlayerDie(attacker, self, hitLocation, mod);
    }
    // END
    ...
    if (attacker && attacker->client)
    {
        trap_GT_SendEvent(
            ...
            self->client->sess.team,
            attacker->s.number,
            attacker->client->sess.team,
            // OSP
            // 0,
            (attacker->client->ospStats) ? attacker->client->ospStats->x3c : 0
            // END
        );
    }
    else
    ...
    if (!level.warmupTime)
        G_LogPrintf("Kill: %i %i %i: %s killed %s by %s at hit location:%i onsameteam:%i\n",
                    ...
                    obit,
                    hitLocation & ~HL_DISMEMBERBIT,
                    (attacker && attacker->client) ? OnSameTeam(self, attacker) : qfalse);
    // END
    ...
    if (!attacker || !attacker->client || killer == ENTITYNUM_WORLD)
    {
        ent = G_TempEntity(self->r.currentOrigin, EV_OBITUARY);
        ent->s.eventParm = mod;
        ent->s.otherEntityNum = self->s.number;
    ...
    //     break;
    // }
    anim = OSP_DeathAnim(self, (hitLocation & (~HL_DISMEMBERBIT)), hitDir);
    // END
...
int CheckArmor(gentity_t *ent, int damage, int dflags, int location, qboolean heavyWeapon)
// END
{
    gclient_t *client;
    int save;
    ...
    // OSP
    if (osp_modversion.integer >= 2 && heavyWeapon)
    {
        save = ceil(damage * 0.35);
    }
    // END
    ...
    {
        if (count && !heavyWeapon)
        {
            if (location & (HL_LEG_UPPER_RT | HL_LEG_UPPER_LT | HL_WAIST | HL_BACK_RT | HL_BACK_LT | HL_BACK | HL_CHEST_RT | HL_CHEST_LT | HL_CHEST))
            {
                int newSave;
                newSave = rand() % 100;
                if (newSave < count)
...
void G_ApplyKnockback(gentity_t *targ, vec3_t newDir, float knockback)
{
    ...
    if (g_gravity.value > 0)
    {
        ...
        if (osp_realisticMode.integer)
            VectorScale(newDir, (g_knockback.value * 0.25) * (float)knockback / mass * 0.8, kvel);
        else
            VectorScale(newDir, g_knockback.value * (float)knockback / mass * 0.8, kvel);
        // END
    }
    ...
    int location)
{
    ...
    if (osp_realisticMode.integer)
    {
        if ((mod & 0xFF) != MOD_WATER && (mod & 0xFF) != MOD_FALLING)
        {
            if ((mod & 0xFF) == MOD_M15_GRENADE ||
                (mod & 0xFF) == MOD_M84_GRENADE ||
                (mod & 0xFF) == MOD_ANM14_GRENADE ||
                (mod & 0xFF) == MOD_SMOHG92_GRENADE ||
                (mod & 0xFF) == MOD_MDN11_GRENADE ||
                (mod & 0xFF) == MOD_F1_GRENADE ||
                (mod & 0xFF) == MOD_L2A2_GRENADE ||
                (mod & 0xFF) == MOD_M67_GRENADE)
            {
                damage *= 2;
            }
            else if (location & (HL_FOOT_RT | HL_FOOT_LT | HL_LEG_LOWER_RT | HL_LEG_LOWER_LT | HL_HAND_RT | HL_HAND_LT | HL_ARM_RT | HL_ARM_LT))
            {
                damage = 95;
            }
            else
    ...
    if ((mod & 0xFF) < MOD_WATER || (mod & 0xFF) >= MOD_USSOCOM_SILENCED)
    // END
    {
...
*/
    ...
    if (!(dflags & DAMAGE_NO_PROTECTION))
    {
        ...
        if (targ != attacker && OnSameTeam(targ, attacker))
        {
            ...
            if (!g_friendlyFire.integer || (level.warmupTime && g_friendlyFire.integer != 2))
            // END
            {
                return 0;
            }
            ...
            {
                damage *= osp_friendlyFireMultiplier.value;
            }
            // END
        }
    ...
    if (attacker == targ)
    {
        ...
        {
            if ((mod & 0xff) == MOD_M4_ASSAULT_RIFLE && ((mod >> 8) & 0xff) == ATTACK_ALTERNATE)
            {
                take *= 10;
            }
            else if ((mod & 0xff) == MOD_OICW && ((mod >> 8) & 0xff) == ATTACK_ALTERNATE)
            {
                take *= 10;
            }
            else
    ...
    asave = CheckArmor(targ, take, dflags, location, ((mod & 0xff) == MOD_M60_MACHINEGUN || (mod & 0xff) == MOD_MSG90A1_SNIPER_RIFLE) ? qtrue : qfalse);
    // END
    take -= asave;

    // OSP
    OSP_Damage(attacker, targ, asave, location, mod, qfalse);
    // END
    ...
    if (g_teamkillDamageMax.integer && mod != MOD_TELEFRAG && !(dflags & DAMAGE_NO_TEAMKILL))
    {
        ...
        {
            // OSP
            if ((attacker->client && OSP_IsClientAdmin(attacker - g_entities, qtrue)) || osp.matchPhase < 3)
            {
            }
            else
            {
    ...
    {
        // OSP
        OSP_Damage(attacker, targ, (take > targ->health) ? targ->health : take, location, mod, qtrue);
        // END
