...
gspawn_t *G_SelectRandomSpawnPoint(team_t team)
{
    ...
    if (!count)
    {
        ...
        // OSP
        if (OSP_FindSafeSpawnpoint(team))
        {
            return &level.x191c5c;
        }
        // END
...
void respawn(gentity_t *ent)
{
    ...
    // OSP
    if (ent->client->ospClient->jailTime && ent->client->ospClient->jailTime > level.time)
    {

        trap_SendServerCommand(ent - g_entities, "cp \"You have been jailed!\n\"");
        ent->client->ps.respawnTimer = ent->client->ospClient->jailTime + 1000;
        G_StartGhosting(ent);
        return;
    }
    // END
...
void G_UpdateOutfitting(int clientNum)
{
    ...
    // OSP
    client->ps.firemode[27] = 0;
    client->ps.firemode[28] = 0;
    client->ps.firemode[29] = 0;
    client->ps.firemode[30] = 0;
    client->ps.firemode[31] = 0;
    // END
    ...
    if (BG_IsWeaponAvailableForOutfitting(WP_KNIFE, 2))
    {
        ...
        (ammoIndex < MAX_AMMO)
            ? (client->ps.ammo[ammoIndex] = ammoData[ammoIndex].max)
            : (client->ps.firemode[MAX_WEAPONS - 1 - (ammoIndex - MAX_AMMO)] = ammoData[ammoIndex].max);
        // END
    }
    ...
    // OSP
    if (osp.pistolsOnly || osp.weaponOnly)
    {
        // No knife throwing, delete ammo
        (ammoIndex < MAX_AMMO)
            ? (client->ps.ammo[ammoIndex] = 0)
            : (client->ps.firemode[MAX_WEAPONS - 1 - (ammoIndex - MAX_AMMO)] = 0);
    }
    ...
    if (osp.weaponOnly)
    {
        ...
        item = BG_FindWeaponItem(osp.weaponOnly_weapon);

        client->ps.stats[STAT_WEAPONS] |= (1 << item->giTag);
        ammoIndex = weaponData[item->giTag].attack[ATTACK_NORMAL].ammoIndex;
        (ammoIndex < MAX_AMMO)
            ? (client->ps.ammo[ammoIndex] = AMMOWRAP(client->ps, ammoIndex) + weaponData[item->giTag].attack[ATTACK_NORMAL].extraClips * weaponData[item->giTag].attack[ATTACK_NORMAL].clipSize)
            : (client->ps.firemode[MAX_WEAPONS - 1 - (ammoIndex - MAX_AMMO)] = AMMOWRAP(client->ps, ammoIndex) + weaponData[item->giTag].attack[ATTACK_NORMAL].extraClips * weaponData[item->giTag].attack[ATTACK_NORMAL].clipSize);
        ...
        if (weaponData[item->giTag].attack[ATTACK_ALTERNATE].fireAmount && AMMO_NONE != ammoIndex)
        {
            ...
            (ammoIndex < MAX_AMMO)
                ? (client->ps.ammo[ammoIndex] = AMMOWRAP(client->ps, ammoIndex) + weaponData[item->giTag].attack[ATTACK_ALTERNATE].extraClips * weaponData[item->giTag].attack[ATTACK_ALTERNATE].clipSize)
                : (client->ps.firemode[MAX_WEAPONS - 1 - (ammoIndex - MAX_AMMO)] = AMMOWRAP(client->ps, ammoIndex) + weaponData[item->giTag].attack[ATTACK_ALTERNATE].extraClips * weaponData[item->giTag].attack[ATTACK_ALTERNATE].clipSize);
        }
    ...
    for (group = 0; group < OUTFITTING_GROUP_ACCESSORY; group++)
    {
        ...
        if (osp.pistolsOnly && group != OUTFITTING_GROUP_PISTOL)
        {
            continue;
        }
        ...
        if (weaponData[item->giTag].attack[ATTACK_ALTERNATE].fireAmount && AMMO_NONE != ammoIndex)
        {
            ...
            ent->client->hasM203 = OSP_CheckMaxM203(ent);

            if (item->giTag == WP_M4_ASSAULT_RIFLE && !ent->client->hasM203)
            {
                client->ps.clip[ATTACK_ALTERNATE][item->giTag] = 0;
                (ammoIndex < MAX_AMMO)
                    ? (client->ps.ammo[ammoIndex] = AMMOWRAP(client->ps, ammoIndex))
                    : (client->ps.firemode[MAX_WEAPONS - 1 - (ammoIndex - MAX_AMMO)] = AMMOWRAP(client->ps, ammoIndex));
            }
            else
            {
                client->ps.clip[ATTACK_ALTERNATE][item->giTag] = weaponData[item->giTag].attack[ATTACK_ALTERNATE].clipSize;
    ...
    //     break;
    // }
    client->ps.stats[14] = -1;
    client->ps.stats[15] = -1;
    ...
    case MODELINDEX_AMMO:
    {
        weapon_t weapon = OSP_GetPrimaryWeapon(ent);
        int ammoIndex;
        ...
        if (weapon > WP_NONE && weapon < WP_NUM_WEAPONS)
        {
            ...
            (ammoIndex < MAX_AMMO)
                ? (client->ps.ammo[ammoIndex] = AMMOWRAP(client->ps, ammoIndex) + 2.0 * weaponData[weapon].attack[ATTACK_NORMAL].clipSize)
                : (client->ps.firemode[MAX_WEAPONS - 1 - (ammoIndex - MAX_AMMO)] = AMMOWRAP(client->ps, ammoIndex) + 2.0 * weaponData[weapon].attack[ATTACK_NORMAL].clipSize);
            client->ps.stats[14] = ammoIndex;
        }
        else
        {
            client->ps.stats[14] = -1;
...
void ClientUserinfoChanged(int clientNum)
{
    ...
    if (g_coloredTeamplayNames.integer)
    {
        G_ClientCleanName(s, client->pers.netname, sizeof(client->pers.netname), qtrue);

        if (strlen(client->pers.netname) < MAX_NETNAME - 3)
        {
            strcat(client->pers.netname, "^7\0");
        }
        else
        {
            client->pers.netname[MAX_NETNAME - 3] = '^';
            client->pers.netname[MAX_NETNAME - 2] = '7';
            client->pers.netname[MAX_NETNAME - 1] = '\0';
        }
    }
    else
        G_ClientCleanName(s, client->pers.netname, sizeof(client->pers.netname), level.gametypeData->teams ? qfalse : qtrue);
    ...
    // OSP
    if (client->ospClient && client->ospClient->vip)
    {
        if (osp_femaleVIP.integer)
        {
            if (strcmp(level.gametypeTeam[client->sess.team], "marine") == 0)
            {
                client->pers.identity = BG_FindIdentity("NPC_Prometheus_Soldier_Female2/female_prometheus_soldier");
            }
            else if (strcmp(level.gametypeTeam[client->sess.team], "thug") == 0)
            {
                client->pers.identity = BG_FindIdentity("NPC_HongKong_Thug_Female/hk_female_thug_a1");
            }
            else if (strcmp(level.gametypeTeam[client->sess.team], "hospital") == 0)
            {
                client->pers.identity = BG_FindIdentity("NPC_Female_Civilian/female_civilian");
            }
            else if (strcmp(level.gametypeTeam[client->sess.team], "prometheus") == 0)
            {
                client->pers.identity = BG_FindIdentity("NPC_Prometheus_Soldier_Female/female_prometheus_armor");
            }
            else if (strcmp(level.gametypeTeam[client->sess.team], "civilian") == 0)
            {
                client->pers.identity = BG_FindIdentity("NPC_Female_Civilian/female_civilian_h2");
            }
            else if (strcmp(level.gametypeTeam[client->sess.team], "columbian") == 0)
            {
                client->pers.identity = BG_FindIdentity("NPC_Deviant1/deviant1");
            }
            else if (strcmp(level.gametypeTeam[client->sess.team], "finca") == 0)
            {
                client->pers.identity = BG_FindIdentity("NPC_Finca_Maid/finca_maid");
            }
            else if (strcmp(level.gametypeTeam[client->sess.team], "snow") == 0)
            {
                client->pers.identity = BG_FindIdentity("NPC_Nurse/nurse_w1");
            }
            else if (strcmp(level.gametypeTeam[client->sess.team], "shop") == 0)
            {
                client->pers.identity = BG_FindIdentity("NPC_Taylor_Urban/taylor_armor");
            }
            else
            {
                client->pers.identity = BG_FindIdentity("dmitri");
            }
        }
        else
        {
            if (client->sess.team == TEAM_RED || client->sess.team == TEAM_BLUE)
            {
                if (strcmp(level.gametypeTeam[client->sess.team], "marine") == 0)
                {
                    client->pers.identity = BG_FindIdentity("NPC_Marine_Pilot/skip");
                }
                else if (strcmp(level.gametypeTeam[client->sess.team], "thug") == 0)
                {
                    client->pers.identity = BG_FindIdentity("NPC_Interrogator/interrogator");
                }
                else if (strcmp(level.gametypeTeam[client->sess.team], "hospital") == 0)
                {
                    client->pers.identity = BG_FindIdentity("NPC_Stephan_Fritsch/stephan_fritsch");
                }
                else if (strcmp(level.gametypeTeam[client->sess.team], "prometheus") == 0)
                {
                    client->pers.identity = BG_FindIdentity("NPC_Order_Soldier/order_soldier");
                }
                else if (strcmp(level.gametypeTeam[client->sess.team], "civilian") == 0)
                {
                    client->pers.identity = BG_FindIdentity("NPC_DeadMeat/deadmeat2");
                }
                else if (strcmp(level.gametypeTeam[client->sess.team], "columbian") == 0)
                {
                    client->pers.identity = BG_FindIdentity("NPC_Czech_Soldier/czech_soldier_w1");
                }
                else if (strcmp(level.gametypeTeam[client->sess.team], "finca") == 0)
                {
                    client->pers.identity = BG_FindIdentity("NPC_Priest/priest");
                }
                else if (strcmp(level.gametypeTeam[client->sess.team], "snow") == 0)
                {
                    client->pers.identity = BG_FindIdentity("dmitri");
                ...
                {
                    client->pers.identity = BG_FindIdentity("NPC_Sam/sam_gladstone");
                }
                else
                {
                    client->pers.identity = BG_FindIdentity("dmitri");
    ...
    else if (client->ospClient && client->ospClient->oldVip)
    {
        client->pers.identity = client->pers.cachedIdentity;
    }
    else
        // END
    ...
    // OSP
    if (client->ospClient && !client->ospClient->vip)
        client->pers.cachedIdentity = client->pers.identity;
    // END
    ...
    if (client->pers.connected == CON_CONNECTED)
    {
        ...
        if (client->pers.identity && oldidentity && client->pers.identity != oldidentity && team != TEAM_SPECTATOR)
        {
            ...
            if (!(client->ospClient && client->ospClient->vip) && !(client->ospClient && client->ospClient->oldVip))
                trap_SendServerCommand(-1, va("print \"%s" S_COLOR_WHITE " has changed identities\n\"", client->pers.netname));

            if (client->ospClient && client->ospClient->oldVip)
                client->ospClient->oldVip = qfalse;
            // END
        }
        ...
        if (strcmp(oldname, client->pers.netname) && osp.x646e6c)
        // END
        {
    ...
    // OSP
    s = va("%s%s", s, OSP_ClientUserinfoChanged(clientNum));
    // END
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot)
{
    ...
    // OSP
    firstTime = OSP_ClientConnect(clientNum);
    // END
...
void ClientSpawn(gentity_t *ent)
{
    ...
    // OSP
    OSP_SetCachedGuid(ent - g_entities);
    // END
    ...
    else
    {
        ...
        (start_ammo_type < MAX_AMMO)
            ? (client->ps.ammo[start_ammo_type] = weaponData[WP_USSOCOM_PISTOL].attack[ATTACK_NORMAL].clipSize)
            : (client->ps.firemode[MAX_WEAPONS - 1 - (start_ammo_type - MAX_AMMO)] = weaponData[WP_USSOCOM_PISTOL].attack[ATTACK_NORMAL].clipSize);
        // END
        ...
        if (AMMO_NONE != start_ammo_type)
        {
            ...
            (start_ammo_type < MAX_AMMO)
                ? (client->ps.ammo[start_ammo_type] = ammoData[start_ammo_type].max)
                : (client->ps.firemode[MAX_WEAPONS - 1 - (start_ammo_type - MAX_AMMO)] = ammoData[start_ammo_type].max);
            // END
        }
...
void ClientDisconnect(int clientNum)
{
    ...
    G_LogPrintf("ClientDisconnected: %i\n", clientNum);

    OSP_ClientDisconnect(clientNum);
    // END
