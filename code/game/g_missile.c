...
void G_ExplodeMissile(gentity_t *ent)
{
    ...
    if (osp_modversion.integer >= 2 && ent->classname && strcmp(ent->classname, "m15") == 0)
    {
        ent->s.eType = ET_MISSILE;
        ent->s.loopSound = G_SoundIndex("sound/ambience/generic/gas_escaping.wav");
    ...
    if (ent->splashDamage)
    {
        ...
        {
            // OSP
            if (osp_modversion.integer >= 2 && strcmp(ent->classname, "m15") == 0)
            {
                G_CreateDamageArea(ent->r.currentOrigin,
                                   ent->parent,
                                   ent->splashDamage,
                                   ent->splashRadius * 0.2f,
                                   30000,
                                   ent->methodOfDeath);
            }
            // END
    ...
    else
    {
        ...
        while (1)
        {
            ...
            if (tr.fraction != 1 && g_entities[tr.entityNum].client && (g_entities[tr.entityNum].client->ps.pm_flags & PMF_LEANING))
            {
                ...
                if (shotgun_debugAdjBBox.integer && level.time - last_showBBoxMissile > 500)
                {
