...
void SP_gametype_player(gentity_t *ent)
{
    ...
    // OSP
    if (osp.altmapFlags & 0x4 && ent->spawnflags & 0x1 ||
        osp.altmapFlags & 0x8 && ent->spawnflags & 0x2)
    {
        if (!osp.x6480a0)
        {
            G_FreeEntity(ent);
...
void G_ResetGametype(qboolean fullRestart)
{
    ...
    case RT_INTERVAL:
        ...
        // OSP
        if (strcmp(g_gametype.string, "vip") == 0)
        {
            osp.x6480e8 = 0;
    ...
    case RT_NONE:
        ...
        if (level.gametypeDelayTime != level.time)
        {
            ...
            //trap_SetConfigstring(CS_GAMETYPE_MESSAGE, va("%i,@Get Ready", level.gametypeDelayTime));
            if (OSP_AltmapExists())
                trap_SetConfigstring(CS_GAMETYPE_MESSAGE, va("%i,@Get Ready (AltMap%d)", level.gametypeDelayTime, g_alternateMap.integer));
            else
                trap_SetConfigstring(CS_GAMETYPE_MESSAGE, va("%i,@Get Ready", level.gametypeDelayTime));
            // END
        }
...
void CheckGametype(void)
{
    ...
    if (level.gametypeData->respawnType == RT_NONE && level.gametypeStartTime)
    {
        ...
        // OSP
        OSP_LastOneAliveWarning(TEAM_RED);
        OSP_LastOneAliveWarning(TEAM_BLUE);
        // END
    ...
    if (strcmp(g_gametype.string, "vip") == 0 &&
        !osp.x6480e8 &&
        level.gametypeDelayTime < level.time)
    {
        osp.x6480e8 = qtrue;
        trap_SendServerCommand(-1, "cp \"@GO!\"");
        OSP_GlobalSound("sound/radio/male/move.mp3");
    }

    if (level.gametypeDelayTime &&
        level.gametypeDelayTime < (level.time - 1500) &&
        !osp.x6480e8 &&
        OSP_AltmapExists() &&
        strcmp(level.gametypeData->name, "inf") == 0)
    {
        int i;
...
int G_GametypeCommand(int cmd, int arg0, int arg1, int arg2, int arg3, int arg4)
{
    ...
    case GTCMD_ADDTEAMSCORE:
        ...
        if(strcmp(g_gametype.string, "vip") == 0)
        {
            ...
            for (i = 0; i < level.maxclients; i++)
            {
                ...
                if (ent->client->sess.team == team)
                {
                    OSP_ClientSound(i, "sound/ctf_win.mp3");
                }
                else
                {
                    OSP_ClientSound(i, "sound/ctf_return.mp3");
                }
            }
        }
        // END
    ...
    case GTCMD_GIVECLIENTITEM:
    {
        ...
        if (item)
        {
            ...
            if (strcmp(g_gametype.string, "vip") == 0)
            {
                OSP_SelectVIP(&level.clients[arg0]);
            }
            // END
        }
        break;
