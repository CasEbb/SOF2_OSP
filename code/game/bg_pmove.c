...
static void PM_GroundTrace(void)
{
    ...
    if (pm->x174)
    {
        ...
        if (trace.fraction != 1.0 && pm->x174 != 1 && pm->x174 < pm->levelTime)
        {
            pm->x174 = 0;
        }
    }
    ...
    {
        vec3_t x6c;
        x6c[0] = pm->ps->origin[0];
        x6c[1] = pm->ps->origin[1] + 15.0;
        x6c[2] = pm->ps->origin[2] - 46.0;
        point[0] = x6c[0];
        point[1] = x6c[1];
        point[2] = x6c[2] - 0.25;

        pm->trace(&trace, x6c, NULL, NULL, point, pm->ps->clientNum, pm->tracemask);

        trace.endpos[1] -= 15.0;

        if (trace.fraction)
        {
            pm->x174 = pm->levelTime + 5000;
...
static void PM_SetWaterLevel(void)
{
    ...
    if (!(pm->ps->pm_flags & PMF_LADDER_JUMP) &&
        (cont & CONTENTS_LADDER) &&
        ((pm->useKeyForLadders && (pm->cmd.buttons & BUTTON_USE)) || !pm->useKeyForLadders))
    // END
    {
        if (pm->ps->ladder == -1)
        {
    ...
    {
        // OSP
        if (pm->ps->ladder >= 0 && pm->useKeyForLadders && !(pm->cmd.buttons & BUTTON_USE))
        {
            pm->ps->pm_flags |= PMF_LADDER_JUMP;
        }
        // END
...
static void PM_Footsteps(void)
{
    ...
    else
    {
        if (!(pm->cmd.buttons & BUTTON_WALKING))
        {
            // OSP
            //PM_BeginZoomOut();
            if (pm->ospModVersion == 0 || pm->ps->weapon != WP_SIG551)
            {
                PM_BeginZoomOut();
            }
            // END
            ...
            if (step > pm->ps->weaponCallbackStep)
            {
                if (!Q_stricmp("fire", note->mNote) || !Q_stricmp("altfire", note->mNote))
                {
                    if (pm->ps->weaponstate == WEAPON_FIRING)
                    {
                        ...
                        if (pm->ospModVersion && (pm->ps->pm_flags & PMF_ZOOMED) && pm->ps->weapon == WP_SIG551)
                        {
                            vec3_t angles;
                            VectorSet(angles, 2.45, 0, 0);
                            PM_Weapon_AddKickAngles(angles);
                        }
                        else
                        {
                            PM_Weapon_AddKickAngles(weaponData[pm->ps->weapon].attack[ATTACK_NORMAL].maxKickAngles);
...
static void PM_BeginWeaponChange(int weapon)
{
    ...
    if (pm->ps->weapon >= WP_M84_GRENADE &&
        pm->ps->weapon <= WP_M67_GRENADE &&
        pm->ps->weapon != WP_OICW &&
        pm->ps->weapon != WP_USSOCOM_SILENCED &&
        pm->ps->clip[ATTACK_NORMAL][pm->ps->weapon] <= 0)
    // END
    {
...
void PM_LoadShell(void)
{
    ...
    (weaponData[pm->ps->weapon].attack[ATTACK_NORMAL].ammoIndex < MAX_AMMO)
        ? (pm->ps->ammo[weaponData[pm->ps->weapon].attack[ATTACK_NORMAL].ammoIndex] = AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_NORMAL].ammoIndex) - 1)
        : (pm->ps->firemode[MAX_WEAPONS - 1 - (weaponData[pm->ps->weapon].attack[ATTACK_NORMAL].ammoIndex - MAX_AMMO)] = AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_NORMAL].ammoIndex) - 1);
    // END
}
...
void PM_StartRefillClip(attackType_t attack)
{
    ...
    if (AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[attack].ammoIndex) < extra)
    // END
    {
        // OSP
        //extra = pm->ps->ammo[weaponData[pm->ps->weapon].attack[attack].ammoIndex];
        extra = AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[attack].ammoIndex);
        // END
    }
    ...
    (weaponData[pm->ps->weapon].attack[attack].ammoIndex < MAX_AMMO)
        ? (pm->ps->ammo[weaponData[pm->ps->weapon].attack[attack].ammoIndex] = AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[attack].ammoIndex) - extra)
        : (pm->ps->firemode[MAX_WEAPONS - 1 - (weaponData[pm->ps->weapon].attack[attack].ammoIndex - MAX_AMMO)] = AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[attack].ammoIndex) - extra);
    // END
...
static void PM_Weapon_AddInaccuracy(attackType_t attack)
{
    ...
    if ((pm->ps->pm_flags & PMF_ZOOMED) && pml.groundPlane)
    {
        ...
        if (pm->ospModVersion == 2 && pm->ps->weapon == WP_OICW)
        {
    ...
    // OSP
    else if (pm->ospModVersion == 2 &&
             (pm->ps->pm_flags & PMF_DUCKED) &&
             !(pm->ps->pm_flags & PMF_LEANING) &&
             pm->ps->weapon == WP_M60_MACHINEGUN &&
             pml.groundPlane)
    {
        pm->ps->inaccuracy += 300.0;
    }
    else if (pm->ospModVersion == 2 &&
             !pml.walking &&
             !(pm->ps->pm_flags & PMF_LEANING) &&
             ...
             pml.groundPlane)
    {
        pm->ps->inaccuracy += 350.0;
             ...
             pml.groundPlane)
    {
        pm->ps->inaccuracy += 500.0;
    }
    else if (pm->ospModVersion == 2 &&
             pm->ps->weapon == WP_M60_MACHINEGUN)
    {
        pm->ps->inaccuracy += 750.0;
    }
    // END
    else
    {
    ...
    if (pm->ospModVersion)
    {
        if (pm->ps->weapon == WP_M590_SHOTGUN || pm->ps->weapon == WP_USAS_12_SHOTGUN)
        {
            pm->ps->inaccuracyTime = 500;
        }
    }
    // END
...
static void PM_Weapon_UpdateInaccuracy(void)
{
    ...
    // OSP
    else if (pm->ospModVersion >= 1)
    {
        if (pm->ps->weapon == WP_M590_SHOTGUN || pm->ps->weapon == WP_USAS_12_SHOTGUN)
        {
            int diff = 500 - pm->ps->inaccuracyTime;
            pm->ps->inaccuracy *= (1 - (diff * diff) / 250000);
        }
        else
        {
...
static void PM_Weapon(void)
{
    ...
    if (!pm->ps->clip[ATTACK_ALTERNATE][pm->ps->weapon] && AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex) > 0)
    // END
    {
    ...
    if (pm->ospModVersion)
    {
        if (pm->ps->weapon == WP_M4_ASSAULT_RIFLE &&
            !pm->ps->clip[ATTACK_ALTERNATE][pm->ps->weapon] &&
            AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex) == -1 &&
            (attackButtons & BUTTON_ALT_ATTACK))
        {
            // TODO
            // [osp] this generates the correct code, but can be written cleaner some way
            (weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex < MAX_AMMO)
                ? (pm->ps->ammo[weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex] = pm->ps->generic1)
                : (pm->ps->firemode[MAX_WEAPONS - 1 - (weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex - MAX_AMMO)] = pm->ps->generic1);

            pm->ps->generic1 = 0;
            PM_StartRefillClip(ATTACK_ALTERNATE);
            return;
        }
        else if (pm->ps->weapon == WP_OICW &&
                 !pm->ps->clip[ATTACK_ALTERNATE][pm->ps->weapon] &&
                 AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex) == -1 &&
                 (attackButtons & BUTTON_ATTACK) &&
                 pm->ps->firemode[WP_OICW] == WP_FIREMODE_20MM)
        {
            (weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex < MAX_AMMO)
                ? (pm->ps->ammo[weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex] = pm->ps->generic1)
                : (pm->ps->firemode[MAX_WEAPONS - 1 - (weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex - MAX_AMMO)] = pm->ps->generic1);
    ...
    if ((pm->ps->weaponstate == WEAPON_RELOADING) || (pm->ps->weaponstate == WEAPON_RELOADING_ALT))
    {
        ...
        case WP_M590_SHOTGUN:
            ...
            //          (pm->ps->ammo[weaponData[pm->ps->weapon].attack[ATTACK_NORMAL].ammoIndex] > 0))
                     ...
                     (AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_NORMAL].ammoIndex) > 0))
            {
                // Load 1 more shell.
                pm->ps->clip[ATTACK_NORMAL][pm->ps->weapon]++;
                // OSP
    ...
    //else if (pm->ps->weapon == WP_KNIFE || (pm->ps->weapon >= WP_RPG7_LAUNCHER && pm->ps->weapon <= WP_M15_GRENADE))
             ...
             (pm->ps->weapon >= WP_RPG7_LAUNCHER &&
              pm->ps->weapon <= WP_M67_GRENADE &&
              pm->ps->weapon != WP_OICW && pm->ps->weapon != WP_USSOCOM_SILENCED))
    // END
    {
        if (pm->ps->clip[ATTACK_NORMAL][pm->ps->weapon] < 1)
        {
            ...
            if (AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_NORMAL].ammoIndex) > 0)
            // END
            {
                // Yes, so reload it.
                PM_StartRefillClip(ATTACK_NORMAL);
    ...
    if (attackButtons & BUTTON_ATTACK)
    {
        ...
        // OSP
        if (pm->ps->weapon == WP_OICW && pm->ps->firemode[WP_OICW] == WP_FIREMODE_20MM)
        {
            altFire = qtrue;
    ...
    else
    {
        ...
        if (pm->ps->weapon == WP_KNIFE && AMMOWRAP(*pm->ps, attackData->ammoIndex) < 1)
        // END
        {
            return;
        }
    ...
    if ((attackData->fireFromClip == 0 && AMMOWRAP(*pm->ps, attackData->ammoIndex) - attackData->fireAmount < 0) ||
        (attackData->fireFromClip > 0 && (*ammoSource) - attackData->fireAmount < 0))
    // END
    {
        ...
        if (AMMOWRAP(*pm->ps, attackData->ammoIndex) > 0)
        // END
        {
    ...
    // OSP
    if (pm->ospModVersion == 2)
    {
        if (pm->ps->weapon == WP_RPG7_LAUNCHER)
        {
            if ((abs(pm->ps->velocity[0]) + abs(pm->ps->velocity[1])) > 10.0 ||
                (pm->ps->pm_flags & PMF_LEANING) ||
                !pml.groundPlane)
            {
                PM_WeaponIdle();
    ...
    if (!attackData->fireFromClip)
    {
        (attackData->ammoIndex < MAX_AMMO)
            ? (pm->ps->ammo[attackData->ammoIndex] = AMMOWRAP(*pm->ps, attackData->ammoIndex) - attackData->fireAmount)
            : (pm->ps->firemode[MAX_WEAPONS - 1 - (attackData->ammoIndex - MAX_AMMO)] = AMMOWRAP(*pm->ps, attackData->ammoIndex) - attackData->fireAmount);
    }
    else
    {
        (*ammoSource) -= attackData->fireAmount;
    ...
    if (pm->ospModVersion)
    {
        if (pm->ps->weapon == WP_M4_ASSAULT_RIFLE)
        {
            if (attackData->fireFromClip == 2 &&
                AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex) > 0)
            {
                pm->ps->generic1 = AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex);

                (weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex < MAX_AMMO)
                    ? (pm->ps->ammo[weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex] = -1)
                    : (pm->ps->firemode[MAX_WEAPONS - 1 - (weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex - MAX_AMMO)] = -1);
            }
        }

        if (pm->ps->weapon == WP_OICW)
        {
            if (attackData->fireFromClip == 2 &&
                AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex) > 0)
            {
    ...
    default:
        ...
        if (pm->ospModVersion && !altFire)
        {
            ...
            case WP_MP5:
                if (pm->ospModVersion == 2)
                {
                    pm->ps->weaponTime *= 0.9;
                }
                break;
            case WP_M3A1_SUBMACHINEGUN:
                if (pm->ospModVersion == 2)
                {
                    pm->ps->weaponTime *= 0.75;
                }
                break;
            case WP_AK74_ASSAULT_RIFLE:
                if (pm->ospModVersion == 2)
                {
                    pm->ps->weaponTime *= 0.85;
                }
                break;
            case WP_SIG551:
                if (pm->ps->pm_flags & PMF_ZOOMED)
...
void PM_AdjustAttackStates(pmove_t *pm)
{
    ...
    if (pm->cmd.buttons & BUTTON_ALT_ATTACK)
    {
        ...
        ammoOk = AMMOWRAP(*pm->ps, weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].ammoIndex) - weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].fireAmount;
        // END
    }
    else
    {
...
void PmoveSingle(pmove_t *pmove)
{
    ...
    if ((!(pm->ospModVersion && pm->ps->weapon == WP_SIG551) && ((pm->ps->pm_flags & PMF_ZOOMED) ||
                                                                 (pm->ps->weaponstate == WEAPON_ZOOMIN))) ||
        (pm->cmd.buttons & (BUTTON_LEAN_LEFT | BUTTON_LEAN_RIGHT)) ||
        (pm->ps->stats[STAT_USEWEAPONDROP]))
    {
