...
static void UI_DrawNetFilter(rectDef_t *rect, qhandle_t font, float scale, vec4_t color)
{
    ...
    UI_DrawText(rect->x, rect->y, font, scale, color, va("%s", serverFilters[ui_serverFilterType.integer].description), 0, 0);
    // END
}
...
static int UI_OwnerDrawWidth(int ownerDraw, qhandle_t font, float scale)
{
    ...
    case UI_NETFILTER:
        ...
        s = va("%s", serverFilters[ui_serverFilterType.integer].description);
        // END
        break;
    ...
    int n;
    {
        char info[MAX_INFO_STRING];
        {
            int count;
            {
                ...
                for (n = 0; n < count; n++)
                {
                    trap_GetConfigString(CS_PLAYERS + n, info, sizeof(info));
                    ...
                    if (info[0])
                    {
                        ...
                        if (team == TEAM_RED)
                        {
                            Com_sprintf(uiInfo.x32bf0[uiInfo.x32bec], 4, "%2d", n);
                            Q_strncpyz(uiInfo.x32bf0[uiInfo.x32bec] + 2, Info_ValueForKey(info, "n"), MAX_NAME_LENGTH);
                            Q_CleanStr(uiInfo.x32bf0[uiInfo.x32bec] + 2);
                            uiInfo.x32bec++;
                        }
                    }
                }
...
void sub_000014a2(void)
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        if (info[0] && n != uiInfo.x303ac)
        {
            team = atoi(Info_ValueForKey(info, "t"));

            if (team == TEAM_RED)
            {
                Com_sprintf(uiInfo.x33c08[uiInfo.x33c04], 4, "%2d", n);
                Q_strncpyz(uiInfo.x33c08[uiInfo.x33c04] + 2, Info_ValueForKey(info, "n"), MAX_NAME_LENGTH);
                Q_CleanStr(uiInfo.x33c08[uiInfo.x33c04] + 2);
                uiInfo.x33c04++;
            }
        }
    }
            ...
            int count;
            {
                ...
                for (n = 0; n < count; n++)
                {
                    ...
                    if (info[0])
                    {
                        ...
                        if (team == TEAM_BLUE)
                        {
                            Com_sprintf(uiInfo.x333fc[uiInfo.x333f8], 4, "%2d", n);
                            Q_strncpyz(uiInfo.x333fc[uiInfo.x333f8] + 2, Info_ValueForKey(info, "n"), MAX_NAME_LENGTH);
                            Q_CleanStr(uiInfo.x333fc[uiInfo.x333f8] + 2);
                            uiInfo.x333f8++;
                        }
                    }
                }
...
void sub_000015da(void)
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        if (info[0] && n != uiInfo.x303ac)
        {
            ...
            if (team == TEAM_BLUE)
            {
                Com_sprintf(uiInfo.x34414[uiInfo.x34410], 4, "%2d", n);
                Q_strncpyz(uiInfo.x34414[uiInfo.x34410] + 2, Info_ValueForKey(info, "n"), MAX_NAME_LENGTH);
                Q_CleanStr(uiInfo.x34414[uiInfo.x34410] + 2);
                uiInfo.x34410++;
            }
        }
    }
            ...
            int count;
            {
                ...
                for (n = 0; n < count; n++)
                {
                    ...
                    if (info[0])
                    {
                        ...
                        {
                            Com_sprintf(uiInfo.x303cc[uiInfo.x303b8], 4, "%2d", n);
                            Q_strncpyz(uiInfo.x303cc[uiInfo.x303b8] + 2, Info_ValueForKey(info, "n"), MAX_NAME_LENGTH);
                            Q_CleanStr(uiInfo.x303cc[uiInfo.x303b8] + 2);
                            uiInfo.x303b8++;
                        }
                    }
                }
...
void sub_00001712(void)
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        if (info[0])
        {
            ...
            if (team == TEAM_SPECTATOR)
            {
                x424 = atoi(Info_ValueForKey(info, "sir"));
                x428 = atoi(Info_ValueForKey(info, "sib"));

                if (x424 == 1)
                {
                    Com_sprintf(uiInfo.x30bcc[uiInfo.x303bc], 4, "%2d", n);
                    Q_strncpyz(uiInfo.x30bcc[uiInfo.x303bc] + 2, Info_ValueForKey(info, "n"), MAX_NAME_LENGTH);
                    Q_CleanStr(uiInfo.x30bcc[uiInfo.x303bc] + 2);
                    uiInfo.x303bc++;
                }
                else if (x424 == 0)
                {
                    Com_sprintf(uiInfo.x313cc[uiInfo.x303c0], 4, "%2d", n);
                    Q_strncpyz(uiInfo.x313cc[uiInfo.x303c0] + 2, Info_ValueForKey(info, "n"), MAX_NAME_LENGTH);
                    Q_CleanStr(uiInfo.x313cc[uiInfo.x303c0] + 2);
                    uiInfo.x303c0++;
                }

                if (x428 == 1)
                {
                    Com_sprintf(uiInfo.x31bcc[uiInfo.x303c4], 4, "%2d", n);
                    Q_strncpyz(uiInfo.x31bcc[uiInfo.x303c4] + 2, Info_ValueForKey(info, "n"), MAX_NAME_LENGTH);
                    Q_CleanStr(uiInfo.x31bcc[uiInfo.x303c4] + 2);
                    uiInfo.x303c4++;
                }
                else if (x428 == 0)
                {
                    Com_sprintf(uiInfo.x323cc[uiInfo.x303c8], 4, "%2d", n);
                    Q_strncpyz(uiInfo.x323cc[uiInfo.x303c8] + 2, Info_ValueForKey(info, "n"), MAX_NAME_LENGTH);
                    Q_CleanStr(uiInfo.x323cc[uiInfo.x303c8] + 2);
                    uiInfo.x303c8++;
                }
            }
        }
...
static void UI_RunMenuScript(const char **args)
{
    ...
    if (String_Parse(args, &name))
    {
        ...
        // OSP
        else if (Q_stricmp(name, "adminPlayerCommand") == 0)
        {
            if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount)
            {
                char clientnum[3];
                ...
                clientnum[2] = 0;
                {
                    idnum = atoi(clientnum);
                }

                switch (uiInfo.x303b0)
                {
                case 0:
                    trap_Cmd_ExecuteText(EXEC_APPEND, va("adm kick %d\n", idnum));
                    break;
                case 1:
                    trap_Cmd_ExecuteText(EXEC_APPEND, va("adm ban %d\n", idnum));
                    break;
                case 2:
                    trap_Cmd_ExecuteText(EXEC_APPEND, va("adm strike %d\n", idnum));
                    break;
                case 3:
                    trap_Cmd_ExecuteText(EXEC_APPEND, va("adm phone %d\n", idnum));
                    break;
                case 4:
                    trap_Cmd_ExecuteText(EXEC_APPEND, va("adm strip %d\n", idnum));
                    break;
                case 5:
                    trap_Cmd_ExecuteText(EXEC_APPEND, va("adm mute %d\n", idnum));
                    break;
                case 6:
                    trap_Cmd_ExecuteText(EXEC_APPEND, va("adm bbq %d\n", idnum));
                    break;
                case 7:
                    trap_Cmd_ExecuteText(EXEC_APPEND, va("adm jail %d\n", idnum));
                    break;
                }
            }
        }
        else if (Q_stricmp(name, "adminMessage") == 0)
        {
            char message[100];
            memset(message, 0, sizeof(message));
            trap_Cvar_VariableStringBuffer("ui_AdminChat", message, sizeof(message));

            switch (uiInfo.x303b4)
            {
            case 0:
            ...
            trap_Cvar_Set("ui_AdminChat", "");
        }
        else if (Q_stricmp(name, "adminPause") == 0)
        {
            trap_Cmd_ExecuteText(EXEC_APPEND, "adm pause\n");
        }
        else if (Q_stricmp(name, "adminResume") == 0)
        {
            trap_Cmd_ExecuteText(EXEC_APPEND, "adm unpause\n");
        }
        else if (Q_stricmp(name, "adminCancelVote") == 0)
        {
            trap_Cmd_ExecuteText(EXEC_APPEND, "adm cancelvote\n");
        }
        else if (Q_stricmp(name, "adminSuspend") == 0)
        {
            trap_Cmd_ExecuteText(EXEC_APPEND, "adm suspend\n");
        }
        else if (Q_stricmp(name, "refereeMessage") == 0)
        {
            char message[100];
            memset(message, 0, sizeof(message));
            trap_Cvar_VariableStringBuffer("ui_RefereeChat", message, sizeof(message));
            trap_Cmd_ExecuteText(EXEC_APPEND, va("ref talk %s", message));
        }
        else if (Q_stricmp(name, "refereeChangeTeamRedName") == 0)
        {
            char name[100];
            memset(name, 0, sizeof(name));
            trap_Cvar_VariableStringBuffer("ui_redteamnameDisplay", name, sizeof(name));
            trap_Cmd_ExecuteText(EXEC_APPEND, va("ref name r \"%s\"", name));
        }
        else if (Q_stricmp(name, "refereeChangeTeamBlueName") == 0)
        {
            char name[100];
            memset(name, 0, sizeof(name));
            trap_Cvar_VariableStringBuffer("ui_blueteamnameDisplay", name, sizeof(name));
            trap_Cmd_ExecuteText(EXEC_APPEND, va("ref name b \"%s\"", name));
        }
        else if (Q_stricmp(name, "refereeTeamRedAdd") == 0)
        {
            if (uiInfo.x32bd4 >= 0 && uiInfo.x32bd4 < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x313cc[uiInfo.x32bd4], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("ref specinvite r %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "refereeTeamRedRemove") == 0)
        {
            if (uiInfo.x32bd0 >= 0 && uiInfo.x32bd0 < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x30bcc[uiInfo.x32bd0], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("ref specuninvite r %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "refereeTeamBlueAdd") == 0)
        {
            if (uiInfo.x32bd8 >= 0 && uiInfo.x32bd8 < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x323cc[uiInfo.x32bdc], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("ref specinvite b %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "refereeTeamBlueRemove") == 0)
        {
            if (uiInfo.x32bd8 >= 0 && uiInfo.x32bd8 < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x31bcc[uiInfo.x32bd8], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("ref specuninvite b %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "teamChangeTeamRedName") == 0)
        {
            char name[100];
            memset(name, 0, sizeof(name));
            trap_Cvar_VariableStringBuffer("ui_redteamnameDisplay", name, sizeof(name));
            trap_Cmd_ExecuteText(EXEC_APPEND, va("tcmd name \"%s\"", name));
        }
        else if (Q_stricmp(name, "teamRedDelegate") == 0)
        {
            if (uiInfo.x33c00 >= 0 && uiInfo.x33c00 < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x33c08[uiInfo.x33c00], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("tcmd delegate %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "teamRedAddToTeam") == 0)
        {
            if (uiInfo.x32bcc >= 0 && uiInfo.x32bcc < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x303cc[uiInfo.x32bcc], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("tcmd invite %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "teamRedRemoveFromTeam") == 0)
        {
            if (uiInfo.x32be8 >= 0 && uiInfo.x32be8 < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x32bf0[uiInfo.x32be8], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("tcmd remove %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "teamRedAdd") == 0)
        {
            if (uiInfo.x32bd4 >= 0 && uiInfo.x32bd4 < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x313cc[uiInfo.x32bd4], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("tcmd specinvite %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "teamRedRemove") == 0)
        {
            if (uiInfo.x32bd0 >= 0 && uiInfo.x32bd0 < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x30bcc[uiInfo.x32bd0], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("tcmd specuninvite %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "teamChangeTeamBlueName") == 0)
        {
            char name[100];
            memset(name, 0, sizeof(name));
            trap_Cvar_VariableStringBuffer("ui_blueteamnameDisplay", name, sizeof(name));
        ...
        else if (Q_stricmp(name, "teamBlueDelegate") == 0)
        {
            if (uiInfo.x3440c >= 0 && uiInfo.x3440c < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x34414[uiInfo.x3440c], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("tcmd delegate %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "teamBlueAddToTeam") == 0)
        {
            if (uiInfo.x32bcc >= 0 && uiInfo.x32bcc < MAX_CLIENTS)
            {
                char clientnum[3];
        ...
        else if (Q_stricmp(name, "teamBlueRemoveFromTeam") == 0)
        {
            if (uiInfo.x333f4 >= 0 && uiInfo.x333f4 < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x333fc[uiInfo.x333f4], 2);
                clientnum[2] = 0;
                trap_Cmd_ExecuteText(EXEC_APPEND, va("tcmd remove %s", clientnum));
            }
        }
        else if (Q_stricmp(name, "teamBlueAdd") == 0)
        {
            if (uiInfo.x32bdc >= 0 && uiInfo.x32bdc < MAX_CLIENTS)
            {
                char clientnum[3];
                strncpy(clientnum, uiInfo.x323cc[uiInfo.x32bdc], 2);
                clientnum[2] = 0;
        ...
        else if (Q_stricmp(name, "teamBlueRemove") == 0)
        {
            if (uiInfo.x32bd8 >= 0 && uiInfo.x32bd8 < MAX_CLIENTS)
            {
                char clientnum[3];
        ...
        else if (Q_stricmp(name, "setActivePanels") == 0)
        {
            qboolean isCaptain = qfalse;
            char teamname[1024];
            char info[1024];
            int grade;
            int team;
            ...
            trap_Cvar_VariableStringBuffer("clientnum", info, sizeof(info));
            if (strlen(info) > 0)
                clientnum = atoi(info);
            else
                clientnum = uiInfo.x303ac;

            trap_GetConfigString(CS_PLAYERS + clientnum, info, sizeof(info));
            grade = atoi(Info_ValueForKey(info, "grade"));
            team = atoi(Info_ValueForKey(info, "t"));

            memset(teamname, 0, sizeof(teamname));

            trap_GetConfigString(CS_RED_TEAM_NAME, teamname, sizeof(teamname));
            trap_Cvar_Set("ui_redteamnameDisplay", teamname);
            trap_GetConfigString(CS_BLUE_TEAM_NAME, teamname, sizeof(teamname));
            trap_Cvar_Set("ui_blueteamnameDisplay", teamname);

            if (grade & 0x1)
            {
                trap_Cvar_Set("ui_info_playerIsAdmin", "1");
                trap_Cvar_Set("ui_info_playerIsReferee", "1");
            }
            else
            {
                trap_Cvar_Set("ui_info_playerIsAdmin", "0");

                if (grade & 0x2)
                    trap_Cvar_Set("ui_info_playerIsReferee", "1");
                else
                    trap_Cvar_Set("ui_info_playerIsReferee", "0");
            }

            if (trap_Cvar_VariableValue("ui_teamcontrols") && (grade & 0x4) && team == TEAM_RED)
            {
                trap_Cvar_Set("ui_info_playerIsRedCaptain", "1");
                isCaptain = qtrue;
            }
            else
            {
                trap_Cvar_Set("ui_info_playerIsRedCaptain", "0");
            }

            if (!isCaptain && trap_Cvar_VariableValue("ui_teamcontrols") && (grade & 0x4) && team == TEAM_BLUE)
            {
                trap_Cvar_Set("ui_info_playerIsBlueCaptain", "1");
                isCaptain = qtrue;
            }
            else
            {
                trap_Cvar_Set("ui_info_playerIsBlueCaptain", "0");
            }

            if (isCaptain)
                trap_Cvar_Set("ui_info_playerIsCaptain", "1");
            else
                trap_Cvar_Set("ui_info_playerIsCaptain", "0");
        }
        // END
        else
        {
...
static int UI_FeederCount(float feederID)
{
    ...
    // OSP
    case FEEDER_PLAYERCOMMANDS:
        return sizeof(ospPlayercommands) / sizeof(ospPlayercommands[0]);

    case FEEDER_ADMIN_CHAT:
        return sizeof(ospAdminChat) / sizeof(ospAdminChat[0]);

    case FEEDER_SPECTATOR_LIST:
        if (uiInfo.uiDC.realTime > uiInfo.x32be0)
        {
            uiInfo.x32be0 = uiInfo.uiDC.realTime + 1000;
            sub_0000167b();
        }

        return uiInfo.x303b8;

    case FEEDER_SPECINVITE_RED:
        if (uiInfo.uiDC.realTime > uiInfo.x32be0)
        {
            uiInfo.x32be0 = uiInfo.uiDC.realTime + 1000;
            sub_00001712();
        ...
        return uiInfo.x303bc;

    case FEEDER_SPECINVITE_BLUE:
        if (uiInfo.uiDC.realTime > uiInfo.x32be0)
        {
            uiInfo.x32be0 = uiInfo.uiDC.realTime + 1000;
            sub_00001712();
        ...
        return uiInfo.x303c4;

    case FEEDER_SPECNOTINVITE_RED:
        if (uiInfo.uiDC.realTime > uiInfo.x32be0)
        {
            uiInfo.x32be0 = uiInfo.uiDC.realTime + 1000;
            sub_00001712();
        ...
        return uiInfo.x303c8;

    case FEEDER_SPECNOTINVITE_BLUE:
        if (uiInfo.uiDC.realTime > uiInfo.x32be0)
        {
            uiInfo.x32be0 = uiInfo.uiDC.realTime + 1000;
            sub_00001712();
    ...
    case FEEDER_TEAMRED_LIST:
        if (uiInfo.uiDC.realTime > uiInfo.x32be4)
        {
            uiInfo.x32be4 = uiInfo.uiDC.realTime + 1000;
            sub_0000140b();
        }

        return uiInfo.x32bec;

    case FEEDER_TEAMBLUE_LIST:
        if (uiInfo.uiDC.realTime > uiInfo.x333f0)
        {
            uiInfo.x333f0 = uiInfo.uiDC.realTime + 1000;
            sub_00001543();
        }

        return uiInfo.x333f8;

    case FEEDER_TEAMRED_DELEGATE_LIST:
        if (uiInfo.uiDC.realTime > uiInfo.x33bfc)
        {
            uiInfo.x33bfc = uiInfo.uiDC.realTime + 1000;
            sub_000014a2();
        }

        return uiInfo.x33c04;

    case FEEDER_TEAMBLUE_DELEGATE_LIST:
        if (uiInfo.uiDC.realTime > uiInfo.x34408)
        {
            uiInfo.x34408 = uiInfo.uiDC.realTime + 1000;
            sub_000015da();
        }

        return uiInfo.x34410;
        // END
    }
...
static const char *UI_FeederItemText(float feederID, int index, int column, qhandle_t *handle)
{
    ...
    *handle = -1;
    ...
    case FEEDER_PLAYERCOMMANDS:
        if (index >= 0 && index < 8)
        {
            return ospPlayercommands[index];
        }
        break;
    ...
    case FEEDER_ADMIN_CHAT:
        if (index >= 0 && index < 2)
        {
            return ospAdminChat[index];
        }
        break;

    case FEEDER_SPECTATOR_LIST:
        if (index >= 0 && index < uiInfo.x303b8)
        {
            return uiInfo.x303cc[index] + 2;
        }
        return "";

    case FEEDER_SPECINVITE_RED:
        if (index >= 0 && index < uiInfo.x303bc)
        {
            return uiInfo.x30bcc[index] + 2;
        }
        return "";

    case FEEDER_SPECINVITE_BLUE:
        if (index >= 0 && index < uiInfo.x303c4)
        {
            return uiInfo.x31bcc[index] + 2;
        }
        return "";

    case FEEDER_SPECNOTINVITE_RED:
        if (index >= 0 && index < uiInfo.x303c0)
        {
            return uiInfo.x313cc[index] + 2;
        }
        return "";

    case FEEDER_SPECNOTINVITE_BLUE:
        if (index >= 0 && index < uiInfo.x303c8)
        {
            return uiInfo.x323cc[index] + 2;
        }
        return "";

    case FEEDER_TEAMRED_LIST:
        if (index >= 0 && index < uiInfo.x32bec)
        {
            return uiInfo.x32bf0[index] + 2;
        }
        return "";

    case FEEDER_TEAMBLUE_LIST:
        if (index >= 0 && index < uiInfo.x333f8)
        {
            return uiInfo.x333fc[index] + 2;
        }
        return "";

    case FEEDER_TEAMRED_DELEGATE_LIST:
        if (index >= 0 && index < uiInfo.x33c04)
        {
            return uiInfo.x33c08[index] + 2;
        }
        return "";

    case FEEDER_TEAMBLUE_DELEGATE_LIST:
        if (index >= 0 && index < uiInfo.x34410)
        {
            return uiInfo.x34414[index] + 2;
        }
        return "";
        // END
    }
...
void _UI_Init(qboolean inGameLoad)
{
    ...
    // OSP
    {
        ...
        char info[MAX_INFO_STRING];
        trap_GetClientState(&cs);
        uiInfo.x303ac = cs.clientNum;
        trap_GetConfigString(CS_SERVERINFO, info, sizeof(info));

        if (atoi(Info_ValueForKey(info, "osp_modversion")) == 2)
        {
