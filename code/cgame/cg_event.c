    ...
    case GAME_OVER_SCORELIMIT:
        if (cgs.gametypeData->teams)
        {
            ...
            case TEAM_RED:
                ...
                Com_sprintf(cgs.gameover, MAX_QPATH, "%s hit the score limit", ospCg.teamNames[TEAM_RED]);
                // END
                break;
            ...
            case TEAM_BLUE:
                ...
                Com_sprintf(cgs.gameover, MAX_QPATH, "%s Team hit the score limit", ospCg.teamNames[TEAM_BLUE]);
                // END
                break;
            }
        }
...
static void CG_Obituary(entityState_t *ent)
{
    ...
    if (attacker != ENTITYNUM_WORLD)
    {
        ...
        if (message)
        {
            ...
            Com_Printf("%s%s %s %s%s%s%s\n", targetColor, targetName, message, attackerColor, attackerName, message2, hitLocation == HL_HEAD ? " ^1[ ^3HeaDShoT ^1]" : "");
            // END
            return;
        }
    }
...
void CG_EntityEvent(centity_t *cent, vec3_t position)
{
    ...
    case EV_BOTWAYPOINT:
        ...
        CG_TestLine(cent->lerpOrigin, es->angles, es->time, es->time2, 3);
        // END
