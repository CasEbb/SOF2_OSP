...
static void CG_DrawTeamScores(float y)
{
    ...
    else
    {
        ...
        s = va("%s: %s  %s: %s", ospCg.teamNames[TEAM_RED], scores[0], ospCg.teamNames[TEAM_BLUE], scores[1]);
        // END
        w = trap_R_GetTextWidth(s, cgs.media.hudFont, 0.35f, 0);
...
void CG_AddLagometerSnapshotInfo(snapshot_t *snap)
{
    ...
    if (cg.demoPlayback)
    {
        lagometer.snapshotSamples[lagometer.snapshotCount & (LAG_SAMPLES - 1)] = snap->serverTime - prevServerTime;
        prevServerTime = snap->serverTime;
    }
    else
    {
        lagometer.snapshotSamples[lagometer.snapshotCount & (LAG_SAMPLES - 1)] = snap->ping;
...
static void CG_DrawHUDIcons(void)
{
    ...
    if (cgs.gametype == BG_FindGametype("inf"))
    {
        return;
    }
    // END
    ...
    for (i = 0; i < MAX_HUDICONS; i++)
    {
        ...
        if (osp_drawTeamOverlay.integer)
            CG_DrawPic(x, 300, 32, 32, cgs.gameIcons[cgs.hudIcons[i]]);
        else
            CG_DrawPic(x, 425, 32, 32, cgs.gameIcons[cgs.hudIcons[i]]);
        // END
...
static void CG_Draw2D(void)
{
    ...
    else
    {
        ...
        {
            // OSP
            if (!cg.showScores && osp_simpleHud.integer)
            {
                OSP_DrawHUD();
            }
            // END
            ...
            if (!cg.showScores)
            {
                ...
                // OSP
                //CG_DrawCrosshairNames();
                if (osp_drawCrosshairInfo.integer)
                {
                    OSP_DrawCrosshairInfo();
                }
                else
                {
                    CG_DrawCrosshairNames();
    ...
    if (!cg.scoreBoardShowing)
    {
        cg.scoreBoardShowing = OSP_DrawStats();
    }
    // END
...
void CG_DrawActive(stereoFrame_t stereoView)
{
    ...
    if (cg.predictedPlayerState.pm_flags & PMF_GOGGLES_ON)
    {
        ...
        case GOGGLES_NIGHTVISION:
            ...
            // OSP
            else if (osp_modversion.integer == 2)
            {
                parm1 = 5000.0;
                parm2 = 1000.0;
            }
        ...
        // OSP
        if (osp_modversion.integer == 2 && cg.predictedPlayerState.stats[STAT_GOGGLES] == GOGGLES_NIGHTVISION)
        {
            trap_R_DrawVisualOverlay(GOGGLES_INFRARED, qtrue, 0.0, 800.0);
        }

        if (osp_modversion.integer == 2 && cg.predictedPlayerState.stats[STAT_GOGGLES] == GOGGLES_INFRARED)
        {
