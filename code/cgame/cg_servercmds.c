...
void CG_ParseServerinfo(void)
{
    ...
    // OSP
    cgs.ospClient = atoi(Info_ValueForKey(info, "sv_ospClient")) ? qtrue : qfalse;
    {
        const char *gameversion = Info_ValueForKey(info, "gameversion");
        if (strlen(gameversion) > 0)
            strncpy(ospCg.gameversion, gameversion + 5, 16);
        else
            memset(ospCg.gameversion, 0, 16);
    }
    ospCg.modifiedFiles = atoi(Info_ValueForKey(info, "sv_modifiedfiles")) ? qtrue : qfalse;
    ospCg.realisticMode = atoi(Info_ValueForKey(info, "osp_realisticMode")) ? qtrue : qfalse;
    // END
    ...
    BG_SetAvailableOutfitting(Info_ValueForKey(info, "g_available"));
...
static void CG_ConfigStringModified(void)
{
    ...
    // OSP
    if (OSP_ConfigStringModified(num))
    {
        return;
    }
    // END
    ...
    else if (num == CS_INTERMISSION)
    {
        ...
        {
            ospCg.awards[AWARD_LETHAL].soundPlayed = qfalse;
            ospCg.awards[AWARD_LETHAL].displayTime = cg.time + 500;
            ospCg.awards[AWARD_HEADSHOTS].soundPlayed = qfalse;
            ospCg.awards[AWARD_HEADSHOTS].displayTime = cg.time + 1000;
            ospCg.awards[AWARD_EXPLOSIVES].soundPlayed = qfalse;
            ospCg.awards[AWARD_EXPLOSIVES].displayTime = cg.time + 1500;
            ospCg.awards[AWARD_CRAZIEST].soundPlayed = qfalse;
            ospCg.awards[AWARD_CRAZIEST].displayTime = cg.time + 2000;
            ospCg.awards[AWARD_SURVIVOR].soundPlayed = qfalse;
            ospCg.awards[AWARD_SURVIVOR].displayTime = cg.time + 2500;
            ospCg.awards[AWARD_DAMAGE].soundPlayed = qfalse;
            ospCg.awards[AWARD_DAMAGE].displayTime = cg.time + 3000;
            ospCg.awards[AWARD_SCORE].soundPlayed = qfalse;
            ospCg.awards[AWARD_SCORE].displayTime = cg.time + 3500;
        }
        else if (cg.intermissionStarted == 3)
        {
            ospCg.awards[BADAWARD_QUITTER].soundPlayed = qfalse;
            ospCg.awards[BADAWARD_QUITTER].displayTime = cg.time + 500;
            ospCg.awards[BADAWARD_LEMON].soundPlayed = qfalse;
            ospCg.awards[BADAWARD_LEMON].displayTime = cg.time + 1000;
            ospCg.awards[BADAWARD_NEWBIE].soundPlayed = qfalse;
            ospCg.awards[BADAWARD_NEWBIE].displayTime = cg.time + 1500;
            ospCg.awards[BADAWARD_FREEKILL].soundPlayed = qfalse;
            ospCg.awards[BADAWARD_FREEKILL].displayTime = cg.time + 2000;
            ospCg.awards[BADAWARD_CROSSEYED].soundPlayed = qfalse;
            ospCg.awards[BADAWARD_CROSSEYED].displayTime = cg.time + 2500;
            ospCg.awards[BADAWARD_SPAMMER].soundPlayed = qfalse;
            ospCg.awards[BADAWARD_SPAMMER].displayTime = cg.time + 3000;
            ospCg.awards[BADAWARD_CHATTYBITCH].soundPlayed = qfalse;
            ospCg.awards[BADAWARD_CHATTYBITCH].displayTime = cg.time + 3500;
        }
        // END
    }
...
*/
    ...
    if (Q_stricmp(cmd, "remapShader") == 0)
    {
        if (trap_Argc() == 4)
        {
            ...
            Q_strncpyz(oldShader, CG_Argv(1), 64);
            Q_strncpyz(newShader, CG_Argv(2), 64);
            Q_strncpyz(timeOffset, CG_Argv(3), 64);

            //trap_R_RemapShader(CG_Argv(1), CG_Argv(2), CG_Argv(3));
            trap_R_RemapShader(oldShader, newShader, timeOffset);
            // END
        }
        return;
    }
