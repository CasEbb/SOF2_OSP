...
static void CG_ScoresDown_f(void)
{
    if (cg.scoresRequestTime + 2000 < cg.time)
    {
        ...
        if (osp_oldScoreboard.integer || !cgs.ospClient)
        {
            trap_SendClientCommand("score");
        }
        else
        {
            trap_SendClientCommand("osp_score");
        }
        // END
...
// OSP
static void OSP_StatsDown_f(void)
{
...
static void OSP_StatsUp_f(void)
{
    if (cg.showStats)
    {
        cg.showStats = qfalse;
...
static void OSP_MultiviewDown_f(void)
{
    cg.multiview = qtrue;
}

static void OSP_MultiviewUp_f(void)
{
    cg.multiview = qfalse;
}
// END
...
static int CG_GetOutfittingGroupFromString(const char *str)
{
    ...
    // OSP
    else if (Q_stricmp(str, "accessory") == 0)
    {
        return OUTFITTING_GROUP_ACCESSORY;
    }
    // END
...
static consoleCommand_t commands[] = {
    ...
    // OSP
    {"startrecording", OSP_StartRecording_f},
    {"stoprecording", OSP_StopRecording_f},
    {"+multiview", OSP_MultiviewDown_f},
    {"-multiview", OSP_MultiviewUp_f},
    // END
};
