...
static void CG_DrawPlayerWeaponAmmo(rectDef_t *rect, qhandle_t font, float scale, vec4_t color)
{
    ...
    value = AMMOWRAP(cg.predictedPlayerState, weaponData[cg.weaponSelect].attack[ATTACK_NORMAL].ammoIndex);
    // END
...
static void CG_DrawPlayerAltWeaponAmmo(rectDef_t *rect, qhandle_t font, float scale, vec4_t color)
{
    ...
    value = AMMOWRAP(cg.predictedPlayerState, weaponData[cg.weaponSelect].attack[ATTACK_ALTERNATE].ammoIndex);
    // END
...
float CG_GetValue(int ownerDraw)
{
    ...
    case CG_PLAYER_WEAPON_AMMO:
        if (cent->currentState.weapon)
        {
            ...
            return AMMOWRAP(*ps, weaponData[cent->currentState.weapon].attack[ATTACK_NORMAL].ammoIndex);
            // END
        }
        break;
    ...
    while (visible)
    {
        ...
        else if (flags & (CG_SHOW_PLAYER_ALT_WEAPONINFO | CG_HIDE_PLAYER_ALT_WEAPONINFO))
        {
            ...
            else
            {
                ...
                if (!AMMOWRAP(cg.predictedPlayerState, weaponData[cg.weaponSelect].attack[ATTACK_ALTERNATE].ammoIndex) &&
                    !cg.predictedPlayerState.clip[ATTACK_ALTERNATE][cg.weaponSelect])
                // END
                {
                    visible = qfalse;
...
static void CG_DrawWeaponList(rectDef_t *rect, qhandle_t font, float scale, vec4_t color)
{
    ...
    for (weaponToDraw = WP_NONE + 1; weaponToDraw < WP_NUM_WEAPONS; weaponToDraw++)
    {
        ...
        else if ((ps->stats[STAT_WEAPONS] & (1 << weaponToDraw)))
        {
            ...
            ammoCount += AMMOWRAP(*ps, weaponData[weaponToDraw].attack[ATTACK_NORMAL].ammoIndex);
            // END
            ammoCount += ps->clip[ATTACK_NORMAL][weaponToDraw];
            // OSP
            //ammoCount += ps->ammo[weaponData[weaponToDraw].attack[ATTACK_ALTERNATE].ammoIndex];
            ammoCount += AMMOWRAP(*ps, weaponData[weaponToDraw].attack[ATTACK_ALTERNATE].ammoIndex);
            // END
    ...
    if (item)
    {
        ...
        if (BG_WeaponHasAlternateAmmo(cg.weaponMenuSelect))
        {
            ...
            s = va("Ammo: %i / %i", cg.predictedPlayerState.clip[ATTACK_NORMAL][cg.weaponMenuSelect] + AMMOWRAP(cg.predictedPlayerState, weaponData[cg.weaponMenuSelect].attack[ATTACK_NORMAL].ammoIndex),
                   cg.predictedPlayerState.clip[ATTACK_ALTERNATE][cg.weaponMenuSelect] + AMMOWRAP(cg.predictedPlayerState, weaponData[cg.weaponMenuSelect].attack[ATTACK_ALTERNATE].ammoIndex));
            // ND
        }
        else
        {
            ...
            s = va("Ammo: %i", cg.predictedPlayerState.clip[ATTACK_NORMAL][cg.weaponMenuSelect] + AMMOWRAP(cg.predictedPlayerState, weaponData[cg.weaponMenuSelect].attack[ATTACK_NORMAL].ammoIndex));
            // END
        }
...
const char *CG_GetGameStatusText()
{
    ...
    else
    {
        ...
        else if (cg.teamScores[0] >= cg.teamScores[1])
        {
            ...
            s = va("%s leads %s, %i to %i", ospCg.teamNames[TEAM_RED], ospCg.teamNames[TEAM_BLUE], cg.teamScores[0], cg.teamScores[1]);
            // END
        }
        else
        {
            ...
            s = va("%s leads %s, %i to %i", ospCg.teamNames[TEAM_BLUE], ospCg.teamNames[TEAM_RED], cg.teamScores[1], cg.teamScores[0]);
            // END
        }
    }
