...
static void CG_TouchItem(centity_t *cent)
{
    ...
    if (!BG_CanItemBeGrabbed(cgs.gametype, &cent->currentState, &cg.predictedPlayerState, osp_modversion.integer == 2 ? qtrue : qfalse))
    // END
    {
        // can't hold it
        return;
    ...
    if (item->giType == IT_WEAPON)
    {
        ...
        // }
        if (AMMOWRAP(cg.predictedPlayerState, item->giTag) == 0)
        {
            (item->giTag < MAX_AMMO)
                ? (cg.predictedPlayerState.ammo[item->giTag] = 1)
                : (cg.predictedPlayerState.firemode[MAX_WEAPONS - 1 - (item->giTag - MAX_AMMO)] = 1);
        }
        // END
    }
}
...
void CG_PredictPlayerState(void)
{
    ...
    for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++)
    {
        ...
        if (osp_modversion.integer)
            cg_pmove.useKeyForLadders = osp_useKeyForLadders.integer;
        else
            cg_pmove.useKeyForLadders = 0;
        cg_pmove.ospModVersion = osp_modversion.integer;
        // END
