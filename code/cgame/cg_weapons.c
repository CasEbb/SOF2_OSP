    ...
    if (cg.time == cent->muzzleFlashTime)
    {
        ...
        if (attackInfo->tracerEffect && !(attackDat->projectileLifetime & PROJECTILE_FIRE))
        {
            ...
            if (rand() % 100 < (cg_tracerChance.value * 100.0f))
            {
                ...
                VectorMA(flash.origin, osp_tracerDistance.value, gun.axis[0], origin);
                // END
...
qboolean CG_WeaponSelectable(int i, qboolean allowEmpty)
{
    ...
    if (BG_WeaponHasAlternateAmmo(i))
    {
        ...
        if (AMMOWRAP(cg.predictedPlayerState, weaponData[i].attack[ATTACK_ALTERNATE].ammoIndex) ||
            cg.predictedPlayerState.clip[ATTACK_ALTERNATE][i])
        // END
        {
            return qtrue;
    ...
    ammo += AMMOWRAP(cg.predictedPlayerState, weaponData[i].attack[ATTACK_NORMAL].ammoIndex);
    // END
...
void CG_NextWeapon(qboolean allowEmpty, int exclude)
{
    ...
    for (i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++)
    {
        // OSP
        // selected++;
        if (selected == WP_M60_MACHINEGUN)
            selected = WP_OICW;
        else if (selected == WP_OICW)
            selected = WP_MM1_GRENADE_LAUNCHER;
        else if (selected == WP_SILVER_TALON)
            selected = WP_USSOCOM_SILENCED;
        else if (selected == WP_USSOCOM_SILENCED)
            selected = WP_M590_SHOTGUN;
        else if (selected == WP_RPG7_LAUNCHER)
            selected = WP_SMOHG92_GRENADE;
        else if (selected == WP_SMOHG92_GRENADE)
            selected = WP_MDN11_GRENADE;
        else if (selected == WP_M67_GRENADE)
            selected = WP_M84_GRENADE;
        else if (selected == WP_M84_GRENADE)
            selected = WP_ANM14_GRENADE;
        else if (selected == WP_M15_GRENADE)
            selected = WP_KNIFE;
        else
            selected++;
        // END
...
void CG_PrevWeapon(qboolean allowEmpty, int exclude)
{
    ...
    for (i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++)
    {
        // OSP
        // selected--;
        if (selected == WP_KNIFE)
            selected = WP_M15_GRENADE;
        else if (selected == WP_MM1_GRENADE_LAUNCHER)
            selected = WP_OICW;
        else if (selected == WP_OICW)
            selected = WP_M60_MACHINEGUN;
        else if (selected == WP_M590_SHOTGUN)
            selected = WP_USSOCOM_SILENCED;
        else if (selected == WP_USSOCOM_SILENCED)
            selected = WP_SILVER_TALON;
        else if (selected == WP_MDN11_GRENADE)
            selected = WP_SMOHG92_GRENADE;
        else if (selected == WP_SMOHG92_GRENADE)
            selected = WP_RPG7_LAUNCHER;
        else if (selected == WP_ANM14_GRENADE)
            selected = WP_M84_GRENADE;
        else if (selected == WP_M84_GRENADE)
            selected = WP_M67_GRENADE;
        else
            selected--;
        // END
...
void CG_PredictedBullet(centity_t *cent, attackType_t attack)
{
    ...
    if (cg_antiLag.integer < 1 ||
        cg_synchronousClients.integer ||
        !cg_impactPrediction.integer ||
        (weaponData[ent->weapon].attack[attack].weaponFlags & PROJECTILE_FIRE))
    {
        return;
    }
    ...
    // OSP
    // detailed = qtrue;
    if (osp_modversion.integer)
        detailed = qtrue;
    else
        detailed = (weaponData[ent->weapon].attack[attack].pellets > 1 || weaponData[ent->weapon].attack[attack].melee) ? qfalse : qtrue;
    // END
    ...
    if (osp_modversion.integer)
    {
        if (weaponData[ent->weapon].attack[attack].mod == MOD_M590_SHOTGUN ||
            weaponData[ent->weapon].attack[attack].mod == MOD_USAS_12_SHOTGUN)
        {
            vec3_t diff;

            if (cg.predictedPlayerState.pm_flags & PMF_JUMPING)
            {
                inaccuracy = 1.0f;
            }
    ...
    for (i = 0; i < pellets; i++)
    {
        ...
        if (osp_modversion.integer)
        {
            BG_CalculateBulletEndpoint(start, fireAngs, x94, weaponData[ent->weapon].attack[attack].melee ? 1.35f * weaponData[ent->weapon].attack[attack].rV.range : weaponData[ent->weapon].attack[attack].rV.range, end, &seed);
        }
        else
        {
            BG_CalculateBulletEndpoint(start, fireAngs, inaccuracy, weaponData[ent->weapon].attack[attack].rV.range, end, &seed);
