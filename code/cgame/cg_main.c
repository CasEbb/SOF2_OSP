...
static cvarTable_t cvarTable[] = {
    ...
    {&cg_tracerChance, "cg_tracerChance", "0.5", CVAR_ARCHIVE | CVAR_LOCK_RANGE, 0.0f, 1.0f},

    // OSP
    {&cl_timenudge, "cl_timenudge", "0", CVAR_CHEAT},
    {&fx_nearCull, "fx_nearCull", "0", CVAR_CHEAT},
    // END
...
void CG_StartMusic(qboolean bForceStart)
{
    ...
    if (!parm2[0])
    {
        // OSP
        //strcpy(parm2, parm1);
        Q_strncpyz(parm2, parm1, sizeof(parm2));
        // END
    }
    if (*parm1 && *parm2)
    {
...
void CG_Init(int serverMessageNum, int serverCommandSequence, int clientNum)
{
    ...
    // OSP
    cgs.media.sfFont = trap_R_RegisterFont("sf");
    cgs.media.baFont = trap_R_RegisterFont("ba");
    // END
    ...
    {
        BG_ParseInviewFile(cgs.pickupsDisabled, qtrue, "data/sof2++.wpn");
    }
    else
    {
        BG_ParseInviewFile(cgs.pickupsDisabled, qfalse, osp_weaponsFile.string);
    }

    if (osp_modversion.integer == 2)
    {
        Q_strncpyz(weaponData[WP_M15_GRENADE].attack[ATTACK_NORMAL].explosionEffect, "effects/explosions/phosphorus_explosion_osp_mp.efx", MAX_QPATH);
        Q_strncpyz(weaponData[WP_M15_GRENADE].attack[ATTACK_ALTERNATE].explosionEffect, "effects/explosions/phosphorus_explosion_osp_mp.efx", MAX_QPATH);
    }
    // END
